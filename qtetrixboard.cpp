...
void QTetrixBoard::start(){
    if(isPaused)
        ...
        qDebug()<<"Dificultad recibida en Board"<<this->dificultBoard;
        isStarted = true;
        ...
        if(this->dificultBoard==3){//bastard
            level=6;
        }else if(this->dificultBoard==2){
            level=3;
        }
        else
            level=1;
...
void QTetrixBoard::paintEvent(QPaintEvent *event){ // MOVIMIENTO DE FIGURAS
    ...
    for(int i=0;i<BoardHeight;i++){
        for(int j=0;j<BoardWidth;j++){
            int forma= formaEn(j,BoardHeight-i-1);
            if(forma != 0)
...
{
    if (!isStarted || isPaused || curPiece.shape() == 0) {
    ...
    case Qt::Key_D:
        ...
        break;
    case Qt::Key_C:
        if(this->dificultBoard==1){//si es facil{
            tryMove(curPiece.changePiece(),curX,curY);
        }
        break;
    default:
        QFrame::keyPressEvent(event);
...
void QTetrixBoard::drawSquare(QPainter &painter, int x, int y, int shape)
{
